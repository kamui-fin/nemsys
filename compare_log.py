...
def parse_emulator_log(line):
    match = re.match(r".*\[INFO\] (\w+)\s+(\w+)\s+A:(\w+) X:(\w+) Y:(\w+) P:(\w+) SP:(\w+)", line)
    if match:
        return [int(i, 16) for i in match.groups()]
    return None

def parse_ground_truth_log(line):
    match = re.match(r"(\w+)\s+(\w+)\s+.*\s+A:(\w+) X:(\w+) Y:(\w+) P:(\w+) SP:(\w+).*", line)
    if match:
        return [int(i, 16) for i in match.groups()]
    return None
...
def compare_logs(emulator_logs, ground_truth_logs):
    ...
    while i < len(emulator_logs) and j < len(ground_truth_logs):
        ...
        if emu_parsed != gt_parsed:
            context[len(discrepancies)] = (emulator_logs[i - 1], ground_truth_logs[j - 1])
            discrepancies.append((emu_log, gt_log))
